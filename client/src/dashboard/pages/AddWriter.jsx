  ...
  return (
    <div className='bg-white rounded-md'>
      <div className='flex justify-between p-4'>
        ...
        <Link className='px-3 py-[6px] bg-blue-500 rounded-sm text-white hover:bg-blue-600' to='/dashboard/writers'>Writers</Link>
      </div>
      <div className='p-4'>
        <form onSubmit={submit}>
          <div className='grid grid-cols-2 gap-x-8 mb-3'>
            ...
            <div className='flex flex-col gap-y-2'>
              ...
              <select onChange={inputHandler} value={state.category} required name='category' id='category' className='px-3 py-2 rounded-md outline-0 border border-gray-300 focus:border-blue-500 h-10' >
                <option value="">---select category---</option>
                <option value="Breaking">Breaking News</option>
                <option value="Panjayath">പഞ്ചായത്തു വാർത്തകൾ</option>
                <option value="Ward">വാർഡ് തല വാർത്തകൾ</option>
                <option value="Kerala">കേരള വാർത്തകൾ</option>
                <option value="Information">അറിയ്പുകൾ</option>
          ...
          <div className='mt-4'>
            <div className='px-3 py-[6px] bg-blue-500 rounded-sm text-white hover:bg-blue-600' ><button disabled={loader} className='px-3 py-[6px] bg-blue-500 rounded-sm text-white hover:bg-blue-600' >{loader ? 'Loading...' : 'Add Writer'}</button></div>
          </div>
        </form>
      </div>
    </div>
