...
const CreateNews = () => {
  ...
  const added = async (e) => {
    ...
    try {
      ...
      formData.append('title').reset()
      formData.append('description').reset()
      formData.append('image').reset()
    } catch (error) {
      setLoader(false)
  ...
  return (
    <div className='bg-white rounded-md'>
      <div className='flex justify-between p-4'>
        ...
        <Link className='px-3 py-[6px] bg-blue-500 rounded-sm text-white hover:bg-blue-600' to='/dashboard/news'>News</Link>
      </div>
      ...
      <div className='p-4'>
        <form onSubmit={added} >
          <div className='flex flex-col gap-y-2 mb-6'>
            ...
            <input required value={title} onChange={(e) => setTitle(e.target.value)} type="text" placeholder='title' name='title' className='px-3 py-2 rounded-md outline-0 border border-gray-300 focus:border-green-500 h-10' id='title' />          </div>
          <div className='mb-6'>
            <div>
              <label htmlFor="img" className={`w-full h-[100px] flex rounded text-[#404040] gap-2 justify-center items-center cursor-pointer border-2 border-dashed`}>
                {
          ...
          <div className='mt-4'>
            <button disabled={loader} className='px-3 py-[6px] bg-blue-500 rounded-sm text-white hover:bg-blue-600' >{loader ? 'loading...' : 'Add News'}</button>
          </div>
        </form>
      </div>
